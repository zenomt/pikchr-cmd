...
* <code>-N <i>mod-string</i></code>  
  Only translate the first diagram that has _mod-string_ as one of its [start delimiter modifiers](#start-delimiter-modifiers). This is usually used along with `-q` and `-b` when generating a standalone SVG file.
* `-h`  
